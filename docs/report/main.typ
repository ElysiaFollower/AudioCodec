...
本项目围绕 speech neural codec 的课程任务展开，目标是在仅面向 `16 kHz` 单声道语音的设定下，实现一个可训练、可评测、可解释的神经音频编解码系统，并在统一 benchmark 上与传统音频压缩算法比较压缩率与重建质量。项目早期从 `Conv + RVQ + Decoder` 的最小闭环出发，快速验证了离散 token 化与波形重建链路的可行性，但同时暴露出明显的电流音和数字伪影，说明仅靠轻量卷积骨干与重建损失无法获得可用级的听感。随后系统升级为 `SEANet + EMA RVQ`，并最终通过引入 `MS-STFT discriminator`、feature matching、梯度 `Balancer` 与更接近 Encodec/Audiocraft 的优化器配方，把全数据训练结果推进到高保真语音重建。
...
本项目的模型演进可以概括为两次关键转折。第一次转折，是从课程约束下的最小 `Conv + RVQ` 基线转向 `SEANet + EMA RVQ` 的更强骨干。第二次转折，是从“仅依赖重建损失”转向“对齐 Encodec/Audiocraft 的训练目标与训练机制”。前者解决的是表达能力与量化稳定性问题，后者解决的则是感知音质与训练动力学问题。
...
从研究同僚的角度，如果只保留最小复现信息，这套系统的核心超参数其实可以压缩成下面四个：`sample_rate = 16 kHz`、`hop_length = 320`、`latent_dim = 128`、`codebook_size = 1024`。其余多码率 operating points 都只是在此基础上改变 `num_quantizers`。这也是本项目认为“码率变化应尽量与骨干/训练目标变化解耦”的主要原因。
...
为了让这一点更具体，有必要把最终 loss 写成近似公式。设生成器输出为 `x_hat`，参考波形为 `x`，则当前 generator 端优化的主要量可以抽象为：

$L_G = 0.1 L_"wave" + 2.0 L_"stft" + 4.0 L_"adv" + 4.0 L_"fm" + L_"commit"$

其中 `L_wave` 是 waveform L1，`L_stft` 是三种 FFT size 上的 multi-scale STFT magnitude + log-magnitude loss，`L_adv` 是生成器对判别器的 hinge adversarial term，`L_fm` 是 feature matching loss，`L_commit` 则对应量化器的 commitment penalty。判别器端的目标则是标准的 hinge discriminator objective，即同时拉开真实音频与重建音频在多尺度 STFT 判别器上的响应。

这里最容易被低估的不是 adversarial 或 feature matching 本身，而是 `Balancer`。如果简单用固定系数把这些 loss 直接相加，某个梯度量级过大的项就可能主导整个更新，进而导致静音塌缩或 feature matching 发散。当前实现里，`Balancer` 的做法是：对每个 loss 分别计算其对共享张量的梯度范数，维护对应的 EMA norm，再按预设权重比例把这些梯度重新缩放后回传。也就是说，报告里的 `0.1 / 2.0 / 4.0 / 4.0` 不应被理解为“直接线性加和的系数”，而应理解为“期望的梯度贡献比例”。这一点是本项目与早期失败实验之间最关键的差异之一。
...
#figure(
  table(
    columns: (1.2fr, 1fr, 1fr, 1.2fr, 2.1fr),
    inset: 5pt,
    stroke: 0.4pt,
    align: center + horizon,
    [Model], [Quantizers], [Nominal kbps], [Best step], [Observation],
    [Neural-12k], [24], [12.0], [91000], [High-fidelity reconstruction; gains after 40k became incremental],
    [Neural-8k], [16], [8.0], [98000], [Stable high-fidelity point with lower bitrate than 12k],
    [Neural-4k], [8], [4.0], [148000], [Still highly intelligible, with slightly stronger low-rate artifacts],
    [Neural-2k], [4], [2.0], [157000], [Content remains clear; occasional timbre drift appears in hard samples],
  ),
...
第二个层面是全数据训练。四条 neural ladder 的最佳验证步数分别落在 `91k / 98k / 148k / 157k` 左右，说明随着码率降低，系统通常需要更长的训练预算才能稳定收敛。以 `12 kbps` 路线为例，验证总损失从 `step 1000` 的 `5.00` 持续下降到 `step 91000` 的 `2.18`；主观试听则表明在 `40000 step` 左右已经达到高保真，之后的继续训练仍有小幅改善。也就是说，最终高保真结果并不是“短训偶然撞出来的样例”，而是长程训练和稳定训练制度共同作用的结果。

== Benchmark Setup

benchmark 使用当前 config 所定义的 deterministic `test` split，总计 `43` 条 speech utterances，采样率统一为 `16 kHz` 单声道。神经编解码器使用四个训练完成的 operating points：`Neural-12k`、`Neural-8k`、`Neural-4k` 和 `Neural-2k`。传统 baseline 则包括 `Opus`、`MP3`、`AAC` 和无损 `FLAC`。其中 `FLAC` 仅作为无损锚点，用于验证评测管线是否存在读写或对齐问题，而不参与低码率 lossy 结论。
...
#figure(
  table(
    ...
    stroke: 0.4pt,
    align: center + horizon,
    [Codec label], [Target point], [Actual bitrate (kbps)],
...
在真正低码率区域，neural codec 展示出了最强的实验价值。`Neural-2k` 的平均 `STOI` 达到 `0.886`，`LSD` 为 `10.08`，`multi_scale_stft` 为 `1.022`。作为对比，名义上的 `Opus-2k` 虽然仍能保留一定可懂度，但其实际码率已经达到 `5.45 kbps`，同时 `STOI` 只有 `0.747`，`LSD` 和 `multi_scale_stft` 也明显更差。也就是说，在真正极低的实际码率下，当前 neural codec 已经能够用更少的 bit 维持更强的语音 intelligibility 和更低的频谱失真。

中等码率区域的结果更细腻。`Neural-8k` 对 `MP3` 和 `AAC` 的优势十分明显，尤其在 `LSD` 和 `multi_scale_stft` 上远好于传统实现；但面对 `Opus` 时，情况并不是全面领先。`Opus-8k` 的 `STOI` 仍然高于 `Neural-8k`，表明 Opus 在 speech low-bitrate 场景依然是一个非常强的传统基线。类似地，在 `12 kbps` 左右，`Opus-12k` 在当前客观指标上整体优于 `Neural-12k`。因此，本项目的实验结论不应写成“neural codec 在所有低码率区间全面战胜传统 codec”，更稳妥的表述是：neural codec 在极低实际码率下展示出显著优势，在更高码率下达到与主流传统 codec 可比的高保真水平，而 `Opus` 在 `8-12 kbps` 区间仍然表现非常强势。
...
用户在 benchmark 抽样试听中指出，传统 codec 的 `default` operating point 与原声几乎难以区分，而这与实际码率也完全一致：`mp3-default`、`opus-default` 和 `aac-default` 的实际码率分别约为 `24.29`、`63.06` 和 `74.99 kbps`。因此，default mode 的意义并不在于“证明传统 codec 更强”，而在于揭示 codec-native operating point 会自然落在更高比特预算上，从而提供更接近无损的听感。
...
_图像占位建议：在最终版报告中，可补充一张针对 `1069-133709-0005` 或 `1069-133709-0027` 的 source / neural-2k 频谱对比图，重点展示后半段声线变化对应的高频结构差异。_
...
如果继续推进，本项目最自然的下一步有四个方向。第一，补充更强的感知指标和更系统的主观听测，把当前“试听成功”的结论扩展为更正式的听觉评价。第二，在不改变当前主骨干的前提下，继续研究极低码率区域的 timbre 保真问题，尤其是 `2 kbps` 下的 speaker color drift。第三，完善 benchmark 图表和案例库，例如加入失败样本的频谱对比图与更系统的 artifact taxonomy。第四，如果把项目从课程原型继续推进到研究原型，可以尝试引入更强的熵模型、更多数据或更强的时序建模，以进一步提升同码率下对 `Opus` 的竞争力。
